...
##                                                                            ##
##                       SIMULATIONS AVEC TERME FORCANT                       ##
################################################################################
################################################################################
...
def reestimation_globale_forcant(x_exact=False, theta_exact=False, affichage=False):
    ...
    contrainte = np.eye(3*K+2*N*K) * lamb/sig
    M = csr_matrix(np.append(M, contrainte, axis=0))
    
    b = np.zeros((K*(K-1)//2*N+3*K+2*N*K))
    b[:K*(K-1)//2*N] = -F(C_reel, theta_0, x_0, f)
...
def reestimation_x_forcant(theta_exact=False, affichage=False):
    ...
    B = matrice_B(C_reel, theta_0, x_0, f)
    
    # ajout du terme forcant sur la matrice M
    contrainte = np.eye(2*N*K) * lamb_x/sigma_x
    M = csr_matrix(np.append(B, contrainte, axis=0))
    
    b = np.zeros((K*(K-1)//2*N+2*N*K))
    b[:K*(K-1)//2*N] = -F(C_reel, theta_0, x_0, f)
...
def reestimation_theta_forcant(x_exact=False, affichage=False):
    ...
    A = matrice_A(C_reel, theta_0, x_0, f)
    
    # ajout du terme forcant sur la matrice M
    contrainte = np.eye(3*K) * lamb_theta/sigma_theta
    M = csr_matrix(np.append(A, contrainte, axis=0))
    
    b = np.zeros((K*(K-1)//2*N+3*K))
    b[:K*(K-1)//2*N] = -F(C_reel, theta_0, x_0, f)
...
for n in range(Nexp):
    C_reel, X_reel, theta_reel, theta_0, theta_a, x_reel, x_0, x_a = reestimation_globale_forcant()
...
for N in liste_N_x:
    ...
    for exp in range(Nexp):
        print(exp)
        C_reel, X_reel, theta_reel, theta_0, theta_a, x_reel, x_0, x_a = reestimation_x_forcant(theta_exact=True)
...
for N in liste_N_x:
    ...
    for exp in range(Nexp):
        print(exp)
        C_reel, X_reel, theta_reel, theta_0, theta_a, x_reel, x_0, x_a = reestimation_x_forcant()
...
for N in liste_N_theta:
    ...
    for exp in range(Nexp):
        print(exp)
        C_reel, X_reel, theta_reel, theta_0, theta_a, x_reel, x_0, x_a = reestimation_theta_forcant(x_exact=True)
...
for N in liste_N_theta:
    ...
    for exp in range(Nexp):
        print(exp)
        C_reel, X_reel, theta_reel, theta_0, theta_a, x_reel, x_0, x_a = reestimation_theta_forcant()
...
def matrices_A_B_M(x_exact=False, theta_exact=False):
    ...
    if not x_exact: x_0 = x_reel + normal(0, sigma_x, (N, K, 2))
    if not theta_exact: theta_0 = theta_reel + normal(0, sigma_theta, (K, 3))
